<x-action-section>
    <x-slot name="title">
        {{ __('common.browser_sessions') }}
    </x-slot>

    <x-slot name="description">
        {{ __('common.browser_sessions_text1') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('common.browser_sessions_text2') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('common.this_device') }}</span>
                                    @else
        ...
        <div class="flex items-center mt-5">
            <x-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('common.browser_sessions_logout_button') }}
            </x-button>

            <x-action-message class="ml-3 text-green-500" on="loggedOut">
                {{ __('common.done') }}
            </x-action-message>
        </div>
        ...
        <x-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('common.browser_sessions_modal_password_confirm') }}
