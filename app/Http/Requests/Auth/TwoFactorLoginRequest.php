...
class TwoFactorLoginRequest extends FormRequest
{
    ...
    {
        return $this->session()->has('web.id') &&
            User::find($this->session()->get('web.id'));
    }
    ...
    public function challengedUser()
    {
        ...
        if (! $this->session()->has('web.id') ||
            ! $user = User::find($this->session()->get('web.id'))) {
            return redirect()->route('.2fa')->withErrors(['code' => 'The provided two factor authentication code was invalid.']);
        }
    ...
    {
        if (! $this->remember) {
            $this->remember = $this->session()->pull('web.remember', false);
        }
